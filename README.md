...
El paquete Modbus conserva los SIDs iniciales `1100101-1100102` y reserva `1111001-1111199` para su expansión. Las 112 firmas ampliadas ocupan `1111001-1111112` y cubren conexiones y tasas de solicitud, funciones de lectura/escritura/diagnóstico, respuestas de excepción, códigos de función reservados y Unit IDs inusuales. Los SIDs `1201xxx` propuestos se remapean al bloque SIS para evitar conflictos con otros paquetes.

El paquete Otros conserva los SIDs iniciales `1100801-1100802` y asigna sus 68 firmas ampliadas consecutivamente a `1100803-1100870`, dentro del bloque reservado `1100801-1100899`. Su cobertura incluye floods y escaneos, herramientas ofensivas y C2, correo y protocolos heredados, Redis/MongoDB/Elasticsearch, APIs Docker/Kubernetes y protocolos de amplificación. Los SIDs `1208xxx` propuestos se remapean al bloque SIS y el patrón EICAR representa la barra inversa como `|5C|` para mantener compatibilidad con Snort.

El paquete Windows conserva los SIDs iniciales `1100301-1100302` y asigna sus 83 firmas ampliadas consecutivamente a `1100303-1100385`, dentro del bloque reservado `1100301-1100399`. La cobertura incluye exposición RDP/WinRM/MSRPC/LDAP, PowerShell y LOLBins, descubrimiento de Active Directory, dumping de credenciales, borrado de rastros, preparación de ransomware y marcadores C2. Los SIDs `1203xxx` propuestos se remapean al bloque SIS; las comillas y barras inversas de los patrones se codifican como `|22|` y `|5C|` para que Snort los interprete literalmente.

El paquete SMB conserva los SIDs iniciales `1100701-1100702` y asigna sus 54 firmas ampliadas consecutivamente a `1100703-1100756`, dentro del bloque reservado `1100701-1100799`. La cobertura incluye SMB1/SMB2, tasas de conexión y autenticación, shares administrativas, named pipes, herramientas de movimiento lateral, artefactos de credenciales, scripts/ejecutables, ransomware y operaciones SMB1 sensibles. Los SIDs `1207xxx` propuestos se remapean al bloque SIS y las barras inversas de named pipes se representan como `|5C|` para mantener compatibilidad con Snort.

El paquete Web conserva los SIDs iniciales `1100501-1100502` y reserva `1112001-1112199` para su expansión. Las 192 firmas ampliadas ocupan `1112001-1112192` y cubren traversal y exposición de archivos, SQLi, XSS, command injection, exploits conocidos, webshells, SSRF, open redirects, scanners, métodos HTTP inusuales y DoS web. Los SIDs `1205xxx` propuestos se remapean al bloque SIS; barras, pipes y comillas literales se representan mediante bytes Snort para evitar errores de análisis.
